...
public final class NotificationManager: NSObject {
    ...
    public func scheduleClassReminder(
        ...
        guard
            let reminderDate = Calendar.current.date(
                byAdding: .minute, value: -minutesBefore, to: startTime)
        else {
    ...
    public func scheduleFlashcardReview(
        ...
        content.body =
            "\(dueCount) flashcard\(dueCount == 1 ? "" : "s") in \(className) ready to review"
        content.sound = .default
...
extension Notification.Name {
    public static let notificationTapped = Notification.Name("notificationTapped")
}
